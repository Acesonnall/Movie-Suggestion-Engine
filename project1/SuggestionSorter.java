...
public class SuggestionSorter {
	...
	public int Average(int[][] Data) {
		dataBase = Data; // not sure if this copies the values. do we need to do
							// it in a loop?
		int rating;
		int sum = 0;
		int k = 0;
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 5; j++) {
				// the current sum of ratings for that movie divided by 25 will
				// give us the average.
				rating = Data[i][j];
				sum += rating;
			}
	...
	public int Suggestions() {
		...
		return 0; // Temp return statement to suppress errors
